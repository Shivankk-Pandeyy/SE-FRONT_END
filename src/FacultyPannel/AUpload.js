...
const AUpload = () => {
  ...
  const not2=()=>toast.success("ASSIGNMENT UPLOADED!");
...
const DELPDF=async(pdfID)=>{
  const response=await axios.delete("http://localhost:5000/DELA/"+pdfID);
  not3();
}
  ...
  const submitpdf=async(e)=>{
    ...
    try{
        const response=await axios.post("http://localhost:5000/UA",formData);
        not2()
    }
    catch(err){
        console.log(err);
...
<ToastContainer/>
    ...
    <input type='text' placeholder='ENTER ASSIGNMENT NAME' autoComplete='off' name='name' onChange={(e)=>{
      setPdfInfo({
        name:e.target.value,
        code:id,
        head:TID,
    ...
    <input type="file" accept="application/pdf"  placeholder='Enter PDF' onChange={async(e)=>await setFile(e.target.files[0])}></input>
      ...
      AA.map((val)=>{
        if(val.head===TID && val.code===id){
          return(
            ...
            <button onClick={(e)=>DELPDF(val._id)}>DELETE ASSIGNMENT</button>
